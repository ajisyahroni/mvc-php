...
try {
    ...
    // Tambahkan ini untuk error reporting yang lebih baik (opsional tapi disarankan)
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC); // Mengatur default fetch mode
...
if (!empty($nisn)) {
    // --- Pengambilan Data Peserta UKS ---
    $stmt = $pdo->prepare("SELECT * FROM peserta_uks WHERE NISN = ?");
    $stmt->execute([$nisn]);
...
<body id="main" class="with-login"> <div id="page-wrapper">
        <div id="main-content"> <div id="page-content"> <section class="page" id="list-page-custom"> <div class="bg-light p-3 mb-3"> <div class="container-fluid">
                            <div class="row">
                                <div class="col">
                                    <h4 class="record-title">Pencarian Data NISN</h4> </div>
                            </div>
                        </div>
                    </div>

                    <div class=" animated fadeIn page-content"> <div class="container-fluid">
                            <div class="row">
                                <div class="col-md-12 comp-grid">
                                    <div class="page-report-body"> <form method="get" class="mb-4 d-flex"> <input type="text" name="nisn" class="form-control mr-2" placeholder="Masukkan NISN" value="<?= htmlentities($nisn) ?>">
                                            <button class="btn btn-primary" type="submit">Cari</button>
                                        </form>

                                        <?php if (!empty($nisn)): ?>
                                            <hr class="mb-4"> <h4>👤 Peserta UKS</h4>
                                            <?php if (!empty($peserta)): ?>
                                                <div class="table-responsive"> <table class="table table-bordered table-striped table-sm text-left"> <tr><th>Nama</th><td><?= $peserta[0]['Nama'] ?></td></tr>
                                                        <tr><th>Kelas</th><td><?= $peserta[0]['Kelas'] ?></td></tr>
                                                        <tr><th>Jenis Kelamin</th><td><?= $peserta[0]['Jenis_Kelamin'] ?></td></tr>
                                                        <tr><th>TTL</th><td><?= $peserta[0]['TTL'] ?></td></tr>
                                                        <tr><th>Alergi</th><td><?= $peserta[0]['Alergi'] ?></td></tr>
                                                        <tr><th>Alamat</th><td><?= $peserta[0]['Alamat'] ?></td></tr>
                                                    </table>
                                                </div>
                                            <?php else: ?>
                                            ...
                                            <h4 class="mt-4">📋 MCU Tahunan</h4> <?php if (!empty($mcu)): ?>
                                                <div class="table-responsive">
                                                    <table class="table table-sm table-bordered table-striped text-left"> <thead class="table-header bg-light"> <tr><th>Tanggal</th><th>TB</th><th>BB</th><th>Goldar</th><th>Anggota Tubuh</th><th>Masalah</th><th>Riwayat Kesehatan</th></tr>
                                                        </thead>
                                                        <tbody>
                                                ...
                                                <div class="table-responsive">
                                                    <table class="table table-sm table-bordered table-striped text-left">
                                                        <thead class="table-header bg-light">
                                                            <tr><th>Tanggal</th><th>Subjective</th><th>Objective</th><th>Assesment</th><th>Plan</th></tr>
                                                        </thead>
                                                        <tbody>
    ...
    <script>
        ...
        var csrfToken = 'bc477aa11adc4b26d45d20c693949386'; // Ini mungkin berbeda di tiap sesi
    </script>
